 ...
 * - Inline preview panel (images, PDF iframe, text)
 * - Rename inline
 * - Delete with confirmation
 * - Empty state
 *
 * API wiring (swap TODO comments with real endpoints):
 *   GET  /documents        → list documents
 *   POST /documents/upload → multipart/form-data upload
 *   DELETE /documents/:id  → delete
 *   PATCH /documents/:id   → rename  { name: string }
 */
...
export default function DocumentsPage() {
  ...
  const { data: docs = [], isLoading, isError } = useDocs();
        ...
        <div className="flex-1 min-w-0">
          ...
          {uploads.length > 0 && (
            <div className="mb-4 space-y-2">
              {uploads.map((u) => (
                <div key={u.name} className="bg-white border border-gray-100 rounded-lg px-4 py-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {visible.map((doc) => (
                <div
                  ...
                  className="bg-white border border-gray-100 rounded-xl p-4 cursor-pointer hover:border-cixio-blue hover:shadow-sm transition-all group"
                >
